...
export default class App extends Component {
   ...
   handleInputPin = async (number) => {
    ...
    if( await this.state.pin.length === 4 && await this.state.pin === this.state.validPin){
      ...
      if(await this.state.attempt === 2){
        await this.setState({errorInfo: "PIN incorrect. Next incorrect attempt cause Block."});
        await this.setState({pin: ''}, () => console.log(this.state.pin));
        await this.state.attempt--;
        return
      }
      if(await this.state.attempt <= 1){
        await this.setState({errorInfo: "You are currently block . Please contact or reset"});
        await this.setState({pin: ''}, () => console.log(this.state.pin));
        await this.state.attempt--;
        return
      }
  ...
  renderLogin = () => {
    ...
    return (
      ...
      <Text style={styles.title}>Enter Your Pin No to unlock</Text> 
      <View style={{flexDirection: 'row',  marginTop: 40}}>
        <View style={this.state.pin.length > 0 ? {backgroundColor: '#000', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20} : {borderColor: '#485460', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20}} />
        <View style={this.state.pin.length > 1 ? {backgroundColor: '#000', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20} : {borderColor: '#485460', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20}} />
        <View style={this.state.pin.length > 2 ? {backgroundColor: '#000', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20} : {borderColor: '#485460', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20}} />
        <View style={this.state.pin.length > 3 ? {backgroundColor: '#000', borderWidth: 2, padding: 10, borderRadius: 20, marginRight: 20} : {borderColor: '#485460', borderWidth: 2, padding: 10, borderRadius: 20}} />
      </View>
  ...
  render() {
    return (
      <View style={styles.container} >
        ...
        {this.renderLogin()}
        {this.state.toggleView === false ? <ResetPw toggleViewFunc={this.toggleViewFunc.bind(this)} resetPin={this.resetPin.bind(this)} /> : null}
      </View>
    );
  }
}
